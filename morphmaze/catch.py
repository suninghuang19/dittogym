...
class catch(morphmaze):
    def __init__(self, cfg_path, action_res, action_res_resize, wandb_logger=None, robot_img_path=None, particles_num=15000):
        ...
        print("*******************Morphological Maze CATCH*******************")
        ...
        if not record:
            ...
            while 32 / 128 + 0.001 * i <= 35 / 128:
                self.gui.line(begin=(max(0, 32 / 128 + 0.001 * i - self.anchor[None][0]), 20 / 128),\
                    end=(min(1, 32 / 128 + 0.001 * i - self.anchor[None][0]), 60 / 128), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while -35 / 128 + 0.001 * i <= -32 / 128:
                self.gui.line(begin=(max(0, -35 / 128 + 0.001 * i - self.anchor[None][0]), 20 / 128),\
                    end=(min(1, -35 / 128 + 0.001 * i - self.anchor[None][0]), 60 / 128), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 20 / 128 + 0.001 * i <= 27 / 128:
                self.gui.line(begin=(max(0, (OFFSET + 10) / 128 - self.anchor[None][0]), 20 / 128 + 0.001 * i),\
                    end=(min(1, 32 / 128 - self.anchor[None][0]), 20 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 27 / 128 + 0.001 * i <= 37 / 128:
                self.gui.line(begin=(max(0, (OFFSET + 20) / 128 - self.anchor[None][0]), 27 / 128 + 0.001 * i),\
                    end=(min(1, 32 / 128 - self.anchor[None][0]), 27 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 37 / 128 + 0.001 * i <= 45 / 128:
                self.gui.line(begin=(max(0, (OFFSET -5) / 128 - self.anchor[None][0]), 37 / 128 + 0.001 * i),\
                    end=(min(1, 32 / 128 - self.anchor[None][0]), 37 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 20 / 128 + 0.001 * i <= 27 / 128:
                self.gui.line(begin=(max(0, -32 / 128 - self.anchor[None][0]), 20 / 128 + 0.001 * i),\
                    end=(min(1, (-OFFSET - 20) / 128 - self.anchor[None][0]), 20 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 27 / 128 + 0.001 * i <= 37 / 128:
                self.gui.line(begin=(max(0, -32 / 128 - self.anchor[None][0]), 27 / 128 + 0.001 * i),\
                    end=(min(1, (-OFFSET - 10) / 128 - self.anchor[None][0]), 27 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
            i = 0
            while 37 / 128 + 0.001 * i <= 45 / 128:
                self.gui.line(begin=(max(0, -32 / 128 - self.anchor[None][0]), 37 / 128 + 0.001 * i),\
                    end=(min(1, (-OFFSET - 5) / 128 - self.anchor[None][0]), 37 / 128 + 0.001 * i), radius=3.5, color=0x394C31)
                i += 1  
    ...
    def set_bg_env(self):
        for i, j in self.shape_field:
            ...
            # right wall
            if 37 / 128 < (self.n_grid * 8 - i) / (self.n_grid * 8) < 45 / 128\
                and (OFFSET - 5) / 128 < (j / (self.n_grid * 8) + self.anchor[None][0]) < 32 / 128:
                self.shape_field[i, j] = 1
            if 27 / 128 < (self.n_grid * 8 - i) / (self.n_grid * 8) < 37 / 128\
                and (OFFSET + 20) / 128 < (j / (self.n_grid * 8) + self.anchor[None][0]) < 32 / 128:
                self.shape_field[i, j] = 1
            if 20 / 128 < (self.n_grid * 8 - i) / (self.n_grid * 8) < 27 / 128\
                and (OFFSET + 10) / 128 < (j / (self.n_grid * 8) + self.anchor[None][0]) < 32 / 128:
                self.shape_field[i, j] = 1
            # left wall
            if 37 / 128 < (self.n_grid * 8 - i) / (self.n_grid * 8) < 45 / 128\
                and (-OFFSET - 5) / 128 > (j / (self.n_grid * 8) + self.anchor[None][0]) > -32 / 128:
        ...
        for i, j in self.grid_m:
            ...
            if i > 32 - int(self.anchor[None][0] * self.n_grid) - self.bound\
                and i < 35 - int(self.anchor[None][0] * self.n_grid)\
                    and j < 80 - int(self.anchor[None][1] * self.n_grid) and self.grid_v[i, j][0] > 0:
                self.grid_v[i, j][0] = 0
            if i < -32 - int(self.anchor[None][0] * self.n_grid) + self.bound\
                and i > -35 - int(self.anchor[None][0] * self.n_grid)\
                    and j < 80 - int(self.anchor[None][1] * self.n_grid) and self.grid_v[i, j][0] < 0:
            ...
            # right wall
            # left
            if i > OFFSET - 5 - int(self.anchor[None][0] * self.n_grid) - self.bound\
                and i < OFFSET - 5 + 2 - int(self.anchor[None][0] * self.n_grid)\
                    and 36 - int(self.anchor[None][1] * self.n_grid) < j < 46 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][0] > 0:
                self.grid_v[i, j][0] = 0
            if i > OFFSET + 20 - int(self.anchor[None][0] * self.n_grid) - self.bound\
                and i < OFFSET + 20 + 2 - int(self.anchor[None][0] * self.n_grid)\
                    and 26 - int(self.anchor[None][1] * self.n_grid) < j < 38 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][0] > 0:
                self.grid_v[i, j][0] = 0        
            if i > OFFSET + 10 - int(self.anchor[None][0] * self.n_grid) - self.bound\
                and i < OFFSET + 10 + 2 - int(self.anchor[None][0] * self.n_grid)\
                    and 19 - int(self.anchor[None][1] * self.n_grid) < j < 28 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][0] > 0:
                self.grid_v[i, j][0] = 0  
            # up
            if i > OFFSET - 5 - int(self.anchor[None][0] * self.n_grid)\
                and i < 32 - int(self.anchor[None][0] * self.n_grid)\
                    and 44 - int(self.anchor[None][1] * self.n_grid) < j\
                        < 46 - int(self.anchor[None][1] * self.n_grid) and self.grid_v[i, j][1] < 0:
                self.grid_v[i, j] = [0, 0]
            ...
            if i > OFFSET + 10 - int(self.anchor[None][0] * self.n_grid) and\
                i < OFFSET + 20 - int(self.anchor[None][0] * self.n_grid) and\
                    26 - int(self.anchor[None][1] * self.n_grid) < j < 28 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][1] < 0:
                self.grid_v[i, j][1] = 0
            # down
            if i > OFFSET - 5 - int(self.anchor[None][0] * self.n_grid)\
                and i < OFFSET + 20 - int(self.anchor[None][0] * self.n_grid)\
                    and 36 - int(self.anchor[None][1] * self.n_grid) < j < 38 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][1] > 0:
                self.grid_v[i, j][1] = 0
            ...
            if i < -OFFSET - 5 - int(self.anchor[None][0] * self.n_grid)\
                and i > -32 - int(self.anchor[None][0] * self.n_grid)\
                    and 44 - int(self.anchor[None][1] * self.n_grid) < j < 46 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][1] < 0:
                self.grid_v[i, j] = [0, 0]
                normal = ti.Vector([0.0, 1.0])
                lsq = (normal**2).sum()
            ...
            # down
            if i < -OFFSET - 5 - int(self.anchor[None][0] * self.n_grid)\
                and i > -OFFSET - 10 - int(self.anchor[None][0] * self.n_grid)\
                    and 36 - int(self.anchor[None][1] * self.n_grid) < j < 38 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][1] > 0:
                self.grid_v[i, j][1] = 0
            # down
            if i < -OFFSET - 10 - int(self.anchor[None][0] * self.n_grid)\
                and i > -OFFSET - 20 - int(self.anchor[None][0] * self.n_grid)\
                    and 26 - int(self.anchor[None][1] * self.n_grid) < j < 28 - int(self.anchor[None][1] * self.n_grid)\
                        and self.grid_v[i, j][1] > 0:
                self.grid_v[i, j][1] = 0
