...
class KICK(gym.Env):
    def __init__(
        ...
        self.offset_x = 0.0
        self.offset_y = 0.16
        self.add_voxel(0.0, 0.0, 0.18, 0.18)
        self.object_particles_num = 2000
        self.add_voxel(0.3, 0.0, 0.05, 0.05, is_object=True)
    ...
    def reset(self):
        ...
        if not np.isnan(x_numpy[:self.robot_particles_num, 0]).any():
            ...
            self.anchor[None] = [self.prev_robot_location[0] - 0.3, 0.0]
        return self.state
    ...
    def step(self, action):
        ...
        # # location
        location_reward = 0
        robot_x_mean = curr_robot_location[0]
        ball_x_mean = curr_ball_location[0]
        ...
        robot_location_reward = np.clip(np.sign(robot_x_mean - self.init_robot_location[0]) * (2 * (robot_x_mean - self.init_robot_location[0]))**2 + 4 * (robot_x_mean - self.init_robot_location[0]), a_min=-20, a_max=30)
    ...
    def robot_state(self, state):
        if not np.isnan(self.robot_center_point).any():
            mid_index = [
                int((self.robot_center_point[0] - self.anchor[None][0]) * self.inv_dx * self.obs_res_resize),
                int(OBS_ACT_CENTER * self.inv_dx * self.obs_res_resize),
            ]
        else:
            mid_index = [
                int((self.prev_robot_location[0] - self.anchor[None][0]) * self.inv_dx * self.obs_res_resize),
                int(OBS_ACT_CENTER * self.inv_dx * self.obs_res_resize),
            ]
    ...
    def update_grid_actuation(self, action):
        ...
        if not np.isnan(self.robot_center_point).any():
            base_index = np.array(
                [
                    int((self.robot_center_point[0] - self.anchor[None][0]) * self.inv_dx - 0.5 * final_action_res),
                    int(OBS_ACT_CENTER * self.inv_dx - 0.5 * final_action_res),
                ]
            )
        else:
            base_index = np.array(
                [
                    int((self.prev_robot_location[0] - self.anchor[None][0]) * 0.5 * final_action_res),
                    int(OBS_ACT_CENTER * self.inv_dx - 0.5 * final_action_res),
                ]
            )
...
if __name__ == "__main__":
    ...
    env = KICK(visualize=True, save_file_name="test")
    env.reset()
