...
class SHAPE_MATCH(gym.Env):
    def __init__(
        ...
        self.offset_x = 0.4
        self.offset_y = 0.5
        self.add_voxel(0.0, 0.0, 0.17) 
        self.target_robot = cv2.imread(os.path.join(self.current_directory, "./target_for_shape_match/STAR.jpg"), cv2.IMREAD_GRAYSCALE).astype(np.int32)
    ...
    def step(self, action):
        ...
        # shape
        shape_reward = 0
        new_state = new_state[0].astype(np.int32)
...
if __name__ == "__main__":
    ...
    env = SHAPE_MATCH(visualize=True, save_file_name="test")
    env.reset()
